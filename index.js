// ==========================================
// API KEY MANAGEMENT SYSTEM - MAIN SERVER
// ==========================================
...
// Nota: Este arquivo assume que o TypeScript foi compilado para JavaScript
// Para desenvolvimento com TypeScript, use: npx tsx index.ts

// Verificar se o módulo JavaScript existe
let db;
try {
  db = require('./database/db.js');
} catch (error) {
  console.error('ERRO CRÍTICO: Não foi possível carregar o módulo do banco de dados.');
  console.error('Certifique-se de que o TypeScript foi compilado para JavaScript:');
  console.error('  npx tsc');
  console.error('Ou use tsx para desenvolvimento:');
  console.error('  npx tsx index.ts');
  process.exit(1);
}
...
app.get('/api/admin/keys/:uid', async (req, res) => {
  try {
...
app.put('/api/admin/keys/:uid', async (req, res) => {
  try {
    const { uid } = req.params;
...
app.post('/api/admin/keys/:uid/subscription/cancel', async (req, res) => {
  try {
    const { uid } = req.params;
